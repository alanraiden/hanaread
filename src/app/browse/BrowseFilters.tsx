...
interface Props {
  // SEO: these come pre-fetched from the server component (page.tsx) so the
  // very first paint already has real novel cards in the HTML for Googlebot.
...
export default function BrowseFilters({ initialNovels, initialTotal }: Props) {
  ...
  // We already have correct data for the very first render (it came from the
  // server). Skip the loading skeleton on mount; only show it for subsequent
  // client-side filter/sort/page changes.
  ...
  useEffect(() => {
    ...
    // sort/status/genre/page=1 — don't immediately re-fetch and flash a
    // loading skeleton over content that's already correct.
